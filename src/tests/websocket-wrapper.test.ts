...
if (typeof globalThis.ErrorEvent === 'undefined') {
	...
	globalThis.ErrorEvent = ErrorEvent as typeof globalThis.ErrorEvent
}

/**
 * Very small in-memory stand-in that behaves like the browser WebSocket
 * just enough for WebSocketWrapper. It records every instance so tests
 * can inspect them.
 */
class FakeWebSocket {
	...
	close(code: number = 1000, reason: string = ''): void {
		this.readyState = FakeWebSocket.CLOSED
		if (this.onclose) {
			// The cast is safe for our test purposes
